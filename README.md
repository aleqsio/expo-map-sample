...
comment linking a [visual review](https://app.screenmap.dev) of what changed on-screen.
...
- [.github/workflows/screenmap-baseline.yml](.github/workflows/screenmap-baseline.yml) — on every push
  to `main` (and a weekday cron): refresh the full map incrementally, publish it to the `screenmaps`
  branch, open a flows PR for screens the agent had to explore.
- [.github/workflows/screenmap-pr.yml](.github/workflows/screenmap-pr.yml) — on every PR: capture only
  the screens the diff can affect, pack a `.diff.scrmap`, comment with a preloaded visualiser link.
- [.screenmap/config.json](.screenmap/config.json) — scheme, device, waits, sample params, and the agent
  provider (**opencode** here; `claude` / `codex` / `gemini` work too — set the matching
  `AGENT_API_KEY` repo secret).
- [.screenmap/SKILL.md](.screenmap/SKILL.md) — app-specific guidance for the agent.
...
- `.screenmap/flows/` — replayable navigation flows, committed like code; the baseline job's flows PR
